...
<p>
    <input type="text" id="league_name" onchange="update_tournament(); return false;"
           value="{$oTournament->getLeagueName()}"/> Название лиги (в сети)
</p>
<p>
    <input type="text" id="league_pass" onchange="update_tournament(); return false;"
           value="{$oTournament->getLeaguePass()}"/> Пароль лиги (в сети)
</p>
<p>
...
<p>
    <select id="prolong" onchange="update_tournament(); return false;">
        ...
        {if $aTopics}
            {foreach from=$aTopics item=oTopic name=el2}
                <option value="{$oTopic->getTopicId()}"
                        {if $oTopic->getTopicId()== $oTournament->getProlongTopicId()}selected="selected"{/if}>{$oTopic->getTitle()}</option>
            {/foreach}
        {/if}
    </select> Топик продления
</p>
<p>
    <input type="text" id="datezayavki" onchange="update_tournament(); return false;" name="datezayavki"
           value="{$oTournament->getDatezayavki()|date_format:"%d-%m-%Y"}" class="date-picker"
           data-date-format="dd-mm-yyyy"/> Последний день приема заявок
</p>
<p>
    <input type="text" id="datestart" onchange="update_tournament(); return false;" name="datestart"
           value="{$oTournament->getDatestart()|date_format:"%d-%m-%Y"}" class="date-picker"
           data-date-format="dd-mm-yyyy"/> Старт турнира
</p>
<p>
    <input type="text" id="dateopenrasp" onchange="update_tournament(); return false;" name="dateopenrasp"
           value="{$oTournament->getDateopenrasp()|date_format:"%d-%m-%Y"}" class="date-picker"
           data-date-format="dd-mm-yyyy"/> Открыто расписание (включительно)
</p>
<p>
...
<p>
    <input type="checkbox" id="exist_o" onchange="update_tournament(); return false;" class="checkbox" value="1"
           {if $oTournament->getExistO()}checked="yes"{/if} > Возможен овертайм?
</p>
<p>
...
{if $bIsSuperAdmin}
    ...
    <br/>
    {*
    <a href="#" id="" onclick="ls.au.simple_toggles(this,'createstattable', {literal}{{/literal} tournament: {$oTournament->getTournamentId()} {literal}}{/literal}); return false;">Создать турнирку</a>
    <br/>*}
